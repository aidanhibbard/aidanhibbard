...
Before continuing this piece, I want to give a big shoutout to the folks over at [Railway](https://railway.com)—their recommendations were a huge help.
...
The first sign of the problem was that our app restarted one morning, roughly a month before I wrote this.
...
Okay... We knew this issue wouldn't be fixed within a single sprint, so we scaled up replicas and raised the restart limit before killing the instance.

## Diving in

I went to the Next.js repository to find the methods called out in the stack trace, and [found them in the router](https://github.com/vercel/next.js/blob/ed41ac1a74305c97d281229696ac069556f020f9/packages/next/src/server/lib/router-utils/resolve-routes.ts#L337). Routes are involved? Promises getting rejected? Probably an infinite redirect loop.

The problem was that no users had reported an infinite redirect, and we didn't have any potential culprits on our server.
...
We had one data-hungry component, and that was our sidebar. It had a searchable list of all businesses belonging to a user. The problem was that we never shrunk the data being sent to the client; the query for the sidebar business list included the entire config for every business. It also re-fetched every time you navigated pages since it re-rendered.
...
Okay, shrink the payload and fix any unnecessary re-renders—it resolves the issue, right?
...
So the first request to a webhook when the app came back up would get a response of 200 with the JSON. However, that response was used up by the first request, so every subsequent response only got a code of 200 with no JSON.

So I turned the SUCCESS constant into a simple factory.
...
So it turns out using `next start` means that no Node exit code was being produced, so every time the app restarted it was a gamble on whether Railway would pick it up because there was never a code.

The kind folks over at Railway told us they had seen this kind of thing before, and recommended that Next teams use a single entry point for their app. They pointed us to [their docs for Node SIGTERM events](https://docs.railway.com/guides/nodejs-sigterm).
...
After shipping that new entry point, I was doing some feature development locally with Inngest running in a separate terminal.
...
This lined up perfectly with my previously known facts too, since staging does not have any Inngest jobs running unless we manually trigger them. So while production gets a steady volume of Inngest hits, staging wouldn't have run into that.
...
It was Sentry, or more specifically our Sentry middleware, as we set it up in our Inngest client.
...
I removed the middleware, restarted my Next instance, and left my local Inngest instance running to ping it. I went to grab some food, and when I came back my local instance hadn't restarted.

I created a PR removing the middleware and sent it to production.
...
When pressed for more information, Cursor would always give AN answer but not THE answer, and any ask for an instruction set to reproduce the crashing instance was always met with some hallucinated use case.
...
I found that the best use of your time in these cases is to tackle real known tech debt. Pick a stack trace, an error, something, and work through it.

Either you'll eventually clean all your app's tech debt, or you'll solve the issue—hopefully both.
